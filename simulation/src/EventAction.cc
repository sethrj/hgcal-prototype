...
{
  fMessenger = new G4GenericMessenger(this, "/Simulation/hits/", "Primary generator control");

  // time cut command
  auto& timeCutCmd = fMessenger->DeclarePropertyWithUnit(
    "timeCut", "ns", hitTimeCut, "Size of time window for hit digitalisation");
  ...
  auto& toaThresholdCmd = fMessenger->DeclarePropertyWithUnit(
    "toaThreshold", "keV", toaThreshold, "Threshold for TOA activation");
