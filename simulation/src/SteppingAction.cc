...
void SteppingAction::UserSteppingAction(const G4Step* aStep)
{
  ...
  if (preStepPoint && preStepPoint->GetProcessDefinedStep()) {
    if ((preStepPoint->GetProcessDefinedStep()->GetProcessType() == 4) &&
        (preStepPoint->GetProcessDefinedStep()->GetProcessSubType() == 121))
    {  // Hadronic interaction has process type == 4 and inelastic interaction has
       // SubProcessType==121
  ...
  if (postStepPoint && postStepPoint->GetProcessDefinedStep()) {
    if ((postStepPoint->GetProcessDefinedStep()->GetProcessType() == 4) &&
        (postStepPoint->GetProcessDefinedStep()->GetProcessSubType() == 121))
    {  // Hadronic interaction has process type == 4
