...
RunAction::RunAction(EventAction* eventAction)
  : G4UserRunAction(), fEventAction(eventAction), fOutputFileDir("sim_HGCalTB_G4Standalone")
{
  fMessenger = new G4GenericMessenger(this, "/Simulation/output/", "Output control");
...
void RunAction::BeginOfRunAction(const G4Run* run)
{
  ...
  if (fEventAction) {
    ...
    analysisManager->CreateNtupleDColumn(
      "EdepNonIonizing_keV", fEventAction->hits_EdepNonIonising);  // column Id = 9
    analysisManager->CreateNtupleDColumn("TOA_ns", fEventAction->hits_TOA);  // column Id = 10
    analysisManager->CreateNtupleDColumn(
      "TOA_last_ns", fEventAction->hits_TOA_last);  // column Id = 11
