...
void SiliconPixelHit::Digitise(const G4double timeWindow, const G4double toaThreshold)
{
  ...
  std::sort(eDep.begin(), eDep.end(),
    [](const std::pair<G4double, G4double>& left, const std::pair<G4double, G4double>& right) {
  ...
  for (size_t i = 0; i < eDep.size(); i++) {
    if (timeWindow < 0 || eDep[i].second < firstHitTime + timeWindow) eDep_digi += eDep[i].first;
      ...
      std::cout << "Rejecting hit (" << eDep[i].first << " keV)  at time " << eDep[i].second
                << "(start: " << firstHitTime << ")" << std::endl;
#endif
...
#ifdef DEBUG
  std::cout << timeOfArrival_digi << " to " << timeOfArrival_last_digi
            << "  vs  energy: " << eDep_digi << std::endl;
#endif

  // non ionizing part, does not contribute to TOAs
  if (edep_nonIonizing.size() == 0) return;

  std::sort(edep_nonIonizing.begin(), edep_nonIonizing.end(),
    [](const std::pair<G4double, G4double>& left, const std::pair<G4double, G4double>& right) {
      return left.second < right.second;  // second = time
    });
  ...
  for (size_t i = 0; i < eDep.size(); i++) {
    if (timeWindow == -1 || edep_nonIonizing[i].second < firstHitTime + timeWindow)
      edep_nonIonizing_digi += edep_nonIonizing[i].first;
  }
};