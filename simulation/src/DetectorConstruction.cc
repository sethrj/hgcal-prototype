...
DetectorConstruction::DetectorConstruction()
  : G4VUserDetectorConstruction(), fScoringVolume(0), _configuration(-1)
{
  DefineCommands();
}
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  // World = Beam line
  G4Box* solidWorld =
    new G4Box("World", 0.5 * BEAMLINEXY * m, 0.5 * BEAMLINEXY * m, 0.5 * BEAMLINELENGTH * m);
...
void DetectorConstruction::ConstructHGCal()
{
  ...
  UImanager->ApplyCommand(
    "/vis/viewer/set/targetPoint 0 0 " + std::to_string(default_viewpoint / m) + " m");
...
void DetectorConstruction::ConstructSDandField()
{
  ...
  SiliconPixelSD* sensitiveSilicon =
    new SiliconPixelSD((materials->getSi_pixel_logical()->GetName() + "_sensitive").c_str());
  sdman->AddNewDetector(sensitiveSilicon);
  materials->getSi_pixel_logical()->SetSensitiveDetector(sensitiveSilicon);

  SiPMSD* sensitiveSiPM =
    new SiPMSD((materials->getAHCAL_SiPM_logical()->GetName() + "_sensitive").c_str());
...
void DetectorConstruction::SelectConfiguration(G4int val)
{
  ...
  default_viewpoint = 0;
  if (val == 1)
    defineConfigs1_2_Summer2017(dz_map, default_viewpoint);
  else if (val == 2)
    defineConfigs1_2_Summer2017(dz_map, default_viewpoint);
  else if (val == 3)
    defineConfig3_September2017(dz_map, default_viewpoint);
  else if (val == 15)
    defineDESY2018_Config42(dz_map, default_viewpoint);
  else if (val == 17)
    defineConfigs17To21_June2018(dz_map, default_viewpoint);
  else if (val == 18)
    defineConfigs17To21_June2018(dz_map, default_viewpoint);
  else if (val == 19)
    defineConfigs17To21_June2018(dz_map, default_viewpoint);
    ...
    defineConfig22_October2018_1(dz_map, default_viewpoint);
  else if (val == 23)
    defineConfig23_October2018_2(dz_map, default_viewpoint);
  else if (val == 24)
    defineConfig24_October2018_3(dz_map, default_viewpoint);
  else if (val == 25)
    defineConfig25_October2018_3_parasitic(dz_map, default_viewpoint);
  else if (val == 100)
    defineTestConfig100(dz_map, default_viewpoint);
  else if (val == 101)
    defineTestConfig101(dz_map, default_viewpoint);
  else if (val == 102)
    defineTestConfig102(dz_map, default_viewpoint);
  else if (val == 103)
    defineTestConfig103(dz_map, default_viewpoint);
  else {
...
void DetectorConstruction::DefineCommands()
{
  ...
  fMessenger = new G4GenericMessenger(this, "/Simulation/setup/", "Configuration specifications");

  // configuration command
  auto& configCmd = fMessenger->DeclareMethod("configuration",
    &DetectorConstruction::SelectConfiguration, "Select the configuration (22-24)");
  configCmd.SetParameterName("index", true);
  configCmd.SetDefaultValue("22");

  auto& SiStepSizeCmd =
    fMessenger->DeclareMethod("stepSilicon", &DetectorConstruction::SetStepSizeSilicon,
      "Maximum step size in silicon pixels, unit: microns");
  SiStepSizeCmd.SetParameterName("size", true);
  SiStepSizeCmd.SetDefaultValue("50.");

  auto& MagneticFieldCmd = fMessenger->DeclareMethod("BField",
    &DetectorConstruction::SetMagneticFieldStrength, "Magnetic field strength, unit: Tesla");
